<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CausaExterna.ascx.cs"
    Inherits="WebInternacion2.UserControls.CausaExterna" %>
<script type="text/javascript">
    $(function () {
        var tipoBusqueda = $("#<%= rdbTipoBusqueda.ClientID %> input:checked").val();
        $('#<%= txtAutocie10.ClientID %>').autocomplete({ source: '<%= ResolveUrl("~/Services/CausaExterna.aspx?tipoBusqueda=' + tipoBusqueda + '")%>', minlenght: 3,
...
<table class="autoCompleter">
    <tr>
        <td>
            <asp:TextBox runat="server" ID="txtAutocie10" Columns="58" AutoPostBack="true"></asp:TextBox>
        </td>
        <td>
            <asp:RadioButtonList ID="rdbTipoBusqueda" runat="server" RepeatDirection="Horizontal"
                AutoPostBack="True" Font-Names="Arial" Font-Size="8pt">
